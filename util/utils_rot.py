...
def rotation6d_2_rot_mat(rotation6d):
    ...
    tmp_y = nn.functional.normalize(pose6d[:,3:], dim = -1)
    tmp_z = torch.cross(tmp_y, tmp_x, dim = -1)
...
def rotation6d_2_rot_mat_2(d6: torch.Tensor) -> torch.Tensor:
    a1, a2 = d6[..., :3], d6[..., 3:]
...
def rotation6d_2_rvecs(rotation6d):
    ...
    rot_mats = rotation6d_2_rot_mat_2(rotation6d).reshape((-1, 3, 3))
    rotation6d = rot_mats
    batch_size = rotation6d.shape[0]
